...
#endif
// zapise info o addrese do res deskriptora a nasledne sa ide vytvorit soket na zaklade informacii
    if ((eai = getaddrinfo(address, port, &hints, &res)) != 0 ||
        (res->ai_family != AF_INET && res->ai_family != AF_INET6)) {
    ...
    //tcp_socket = socket(AF_INET, SOCK_STREAM, 0); --> nastavenie na tcp socket 
    //https://www.ibm.com/docs/pl/aix/7.1?topic=protocols-socket-types
    if ((listen_fd = socket(res->ai_family, SOCK_STREAM, IPPROTO_TCP)) == -1 ||
        ...
        //^--povoli opätovné použitie lokálnych adries pre tento soket. Ak povolíte túto možnosť, v skutočnosti môžete mať dva sokety s rovnakým číslom internetového portu;
        // ale systém vám nedovolí použiť dva identicky pomenované sokety spôsobom, ktorý by zmiatol internet.
        // Dôvodom tejto možnosti je, že niektoré internetové protokoly vyššej úrovne vrátane FTP vyžadujú, aby ste stále používali rovnaké číslo portu.
        err = errno; 
...
static int event_loop(Context *context)
{
    ...
    if (fds[POLLFD_LISTENER].revents & POLLIN) {
        new_client_fd = tcp_accept(context, context->listen_fd); //prijatie noveho klienta
        if (new_client_fd == -1) {
            perror("Accepting a new client failed");
            return 0;
        }
        if (context->client_fd != -1) { //ukoncenie stareho klienta
            (void) close(context->client_fd);
            sleep(1);
        }
    ...
    if (fds[POLLFD_TUN].revents & POLLIN) {
        ...
        if (context->client_fd != -1) {
            ...
            writenb = safe_write_partial(context->client_fd, tun_buf.len, 2U + TAG_LEN + len); //zapis zasifrovanych dat z buffra do forwardingu clienta 
...
static int doit(Context *context)
{
    ...
    if (context->is_server) {
        //vytvorenie tcp soketu aj s postupom podla ibm
...
int main(int argc, char *argv[])
{
    ...
    context.is_server = strcmp(argv[1], "server") == 0;         //strcmp ak == tak vrati 0
    if (load_key_file(&context, argv[2]) != 0) {                                            // nacitanie zdielaneho kluca
    ...
    snprintf(context.ext_gw_ip, sizeof context.ext_gw_ip, "%s", ext_gw_ip == NULL ? "" : ext_gw_ip); //zapis gw do struktury
    if (ext_gw_ip == NULL && !context.is_server) { //check of params 
    ...
    get_tun6_addresses(&context); // priradenie ipv6 adries
