    ...
    while (readnb != 0 && count > (ssize_t) 0) {
        while ((readnb = read(fd, buf, count)) < (ssize_t) 0) {
            if (errno == EAGAIN) {
                pfd.fd     = fd;
                pfd.events = POLLIN;
                if (poll(&pfd, (nfds_t) 1, timeout) <= 0) {     //https://man7.org/linux/man-pages/man2/poll.2.html
                    return (ssize_t) -1;
                }
...
const char *get_default_gw_ip(void)
{
    ...
    return read_from_shell_command(gw, sizeof gw,
                                   "ip route show default 2>/dev/null|awk '/default/{print $3}'"); //gw.bat mame pre windows pripraveny bat len ho spustit a presmerovat vystup pripadne overit ci popen ide aj na win 
#else
    return NULL;
#endif
}
...
#elif defined(__linux__)
    return read_from_shell_command(if_name, sizeof if_name,
                                   "ip route show default 2>/dev/null|awk '/default/{print $5}'"); //route print 0* --> a vybrat ip intf 
#else //Get-NetConnectionProfile in PS 
    return NULL;
#endif
}
