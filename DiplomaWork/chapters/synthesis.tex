...
Zdrojový kód implementuje čítania a zápisu dát z TUN tunelu, soketu a príkazového riadku. 
 ...
 \begin{minipage}{\linewidth} 	
 	\begin{lstlisting}[frame=single,
 		numbers=left,
 		caption={Obsah hlavičkového súboru vpn.h}\label{vpn.h},
 		basicstyle=\ttfamily\small, keywordstyle=\color{black}\bfseries,]
...
Vpn.c je hlavným zdrojovým kódom DSVPN. V jeho vnútri nájdeme hlavnú funkciu \lstinline|main()|. Zároveň obsahuje hlavičkové súbory uvedených knižníc. Ako prvé dochádza k inicializácií premennej štruktúry \lstinline|Context| \ref{context}. Štruktúra v~sebe nesie všetky premenné, potrebné na správne fungovanie programu.

\begin{minipage}{\linewidth} 	
	\begin{lstlisting}[frame=single,
		numbers=left,
		caption={Štruktúra Context obsahujúca dôležité premenné programu DSVPN}\label{context},
		basicstyle=\ttfamily\small, keywordstyle=\color{black}\bfseries,]
...
Funkcia \lstinline|permute()| je pomerne rozsiahla. Jej obsah tvorí 100 riadkov zdrojového kódu. Jedná sa o implementáciu XOODOO permutácie, ktorá bola opísaná v Kapitole 2 tejto práce. Použitá implementácia závisí od podmieneného kompilovania zdrojového kódu. Naše zariadenie nemá k dispozícií potrebné procesorové inštrukcie. Po skompilovaní sa používa blok, ktorý sa nachádza v ukážke zdrojového kódu \ref{permute}.   
...
Na VPN klientovi sme spustili v príkazovom riadku príkaz \lstinline|ping 10.8.1.1|. Ten nám vytvára ICMP paket so žiadosťou o odpoveď\footnote{paket číslo 25}. Obsah tohto paketu je možné si pozrieť v obrázku \ref{req}. % TODO: \usepackage{graphicx} required
\begin{figure}[h!]
	\centering
...
Pred samotným používaním soketov je potrebné inicializovať tzv. Winsock. Tento úkon sme realizovali v \lstinline|main()| funkcii, zobrazenú v ukážke zdrojového kódu \ref{wsck}.
